package org.training.constants;

/**
 * Created by nicko on 2/2/2017.
 */
public class PageConstants {
    public static final String VIEW_SIGNIN_JSP = "/view/signin.jsp";
    public static final String VIEW_SIGNUP_JSP = "/view/signup.jsp";
    public static final String VIEW_MENU_JSP = "view/menu.jsp";
    public static final String VIEW_ORDER_JSP = "/view/order.jsp";
    public static final String VIEW_OLD_ORDERS_JSP = "view/oldOrders.jsp";
    public static final String VIEW_ORDERS_JSP = "view/orders.jsp";
    public static final String VIEW_CLOSED_JSP = "view/closed.jsp";
}
